...
The `GDI.py` file contains all functions used for GDI. The core functions are:
  ...
  - `sign_inference(X,M)`: Determine the sign of the relationship between columns of X with history parameter M. Output is
  - `GDI_mask(X,M,B,mask)`: Same as `GDI()`, however . Output is GDI matrix where each element is the GDI from the row to the column.
