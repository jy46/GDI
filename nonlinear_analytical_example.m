...
% GENERATE
X = rand(N,R);
Z = mvnrnd(zeros(1,R),0.25*eye(R),N);   % X has dim: NxR
X(2:end,1)  = sqrt(1-p1)*(Z(2:end,1))  +   sqrt(p1)*(X(1:(end-1),6).^P);
X(2:end,3)  = sqrt(1-p1)*(Z(2:end,3))  +   sqrt(p1)*(X(1:(end-1),6).^P);
X(2:end,5)  = sqrt(1-p1)*(Z(2:end,5))  -   sqrt(p1)*(X(1:(end-1),6).^P);
X(2:end,4)  = sqrt(1-p2)*(Z(2:end,4))  +   sqrt(p2)*(X(1:(end-1),1).^P);
X(2:end,2)  = sqrt(1-p3)*(Z(2:end,2))  ...%+ sqrt(p3/4)*(X(1:(end-1),7).^P)...
                                       - sqrt(p3/3)*(X(1:(end-1),8).^P)...
                                       + sqrt(p3/3)*(X(1:(end-1),9).^P)...
                                       + sqrt(p3/3)*(X(1:(end-1),6).^P);
X(2:end,11) = sqrt(1-p4)*(Z(2:end,11)) +   sqrt(p4)*(X(1:(end-1),4).^P);
X(2:end,10) = sqrt(1-p5)*(Z(2:end,10)) + sqrt(p5/2)*(X(1:(end-1),5).^P)...
                                       + sqrt(p5/2)*(X(1:(end-1),3).^P);                               

% TRUE CONNECTIVITY
true_connectivity = zeros(R,R);
true_connectivity(6,[1 3 5]) = [p1 p1 -p1];
true_connectivity([6 7 9],2) = p3/4;
true_connectivity(8,2) = -p3/4;
...
true_GDI(6,2)  = 0.5*log(1+((p3/4)/(1-p3)));
true_GDI(7,2)  = 0.5*log(1+((p3/4)/(1-p3)));
true_GDI(8,2)  = -0.5*log(1+((p3/4)/(1-p3)));
true_GDI(9,2)  = 0.5*log(1+((p3/4)/(1-p3)));
